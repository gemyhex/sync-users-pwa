  ...
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      ...
      <div>
        ...
        <button @click="triggerSync" class="ml-2 px-3 py-1 bg-blue-600 text-white rounded">Sync Now</button>
      </div>
    </div>
    ...
    <ul>
      <li v-for="u in users" :key="u.id" class="py-2 border-b">
        {{ u.firstName }} {{ masked(u.lastName) }} — <span class="text-sm text-gray-500">ID: {{ u.id }}</span>
      </li>
    </ul>
  </div>
</template>
...
export default defineComponent({
  setup() {
    ...
    function triggerSync() {
      userSyncService.fetchAndStore().then(() => refresh());
    }
